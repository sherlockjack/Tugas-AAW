...
export const verifyAdminTokenService = async (
    ...
    try {
        ...
        if (tenant_id !== SERVER_TENANT_ID) {
            return new UnauthorizedResponse("Invalid token karena Tenant").generate();
        }

        const user = await getUserById(id, SERVER_TENANT_ID);
        if (!user) {
            return new UnauthorizedResponse("Invalid token karena karena").generate();
        }
        ...
        return new UnauthorizedResponse("Invalid token karena lu tolol banget").generate();
    }
}