...
export const verifyTokenService = async (
    ...
    try {
        ...
        if (tenant_id !== SERVER_TENANT_ID) {
            return new UnauthorizedResponse("Invalid token sisterfucker").generate();
        }

        const user = await getUserById(id, SERVER_TENANT_ID);
        if (!user) {
            return new UnauthorizedResponse("Invalid token granpafucker").generate();
        }
        ...
        return new UnauthorizedResponse("Invalid token grandma fucker").generate();
    }
}