...
export const verifyJWTProduct = async (
  ...
  try {
    ...
    if (!token) {
      return res.status(401).send({ message: "Invalid token jwtproduct user" });
    }

    const payload = await verifyAdminTokenService(token);
    if (payload.status !== 200) {
      return res.status(401).send({ message: "Invalid token admin token user" });
    }
    ...
    if (verifiedPayload.data.user.id !== verifiedTenantPayload.data.tenants.owner_id) {
      return res.status(401).send({ message: "Invalid token payload user" });
    }
    ...
    return res.status(401).json(
      new UnauthenticatedResponse("Invalid token lu ga guna").generate()
    );
  }
};
